...
def get_customer(customer_id: str, db: Session = Depends(db_session)) -> dict:
...
def customer_health(customer_id: str, db: Session = Depends(db_session)) -> dict:
    return data_tools.get_health_data(db, customer_id)


@router.get("/{customer_id}/insurance")
def customer_insurance(customer_id: str, db: Session = Depends(db_session)) -> dict:
