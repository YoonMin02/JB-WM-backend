...
class AgentJobDispatcher:
    ...
    def _run_codex_cli(self, input_path: Path, output_path: Path, job_dir: Path) -> dict[str, Any]:
        ...
        prompt = (
            ...
            "plan must include strategy, proposals, explanation, and assessment. "
            "First write plan.strategy as the high-level direction: objective, "
            "priority_order, rationale, and risk_controls. Then create proposals "
            "that follow that strategy. Each proposal must include a customer-readable "
            "execution_summary, concrete execution_steps, and machine-readable "
            "execution_params. Do not jump directly from signal to action without "
            "making the strategy explicit in plan.strategy. In execution_params, "
            "use null for fields that are not relevant to the proposal. "
            ...
            "customer approval and executor verification. For rebalance_portfolio, "
            "set execution_params.target_high_risk_weight and "
            "execution_params.target_low_risk_weight, and describe the sell/buy or "
            "move_to_cash steps in execution_steps using the portfolio allocation "
