...
현재 공식 실행 경로는 LangGraph workflow와 agent job 경계다. 백엔드가 고객
scope, 데이터 수집, 승인, 실행, 감사 로그를 통제하고, 에이전트는 정제된 단일
고객 context를 해석해 필요도와 제안만 만든다.
...
seed는 operator 계정 1개와 고객 계정을 생성한다. 회원가입 API는 MVP 범위에 두지
않고, 데모 고객 데이터와 1:1로 매핑된 계정을 사용한다.

```text
operator@jbwm.local / operator1234
...
프론트는 로그인 응답의 `access_token`을 이후 요청에 붙인다.
...
- `operator`: 전체 고객 조회, workflow event trigger, `/debug` 접근 가능
- `customer`: 자기 `customer_id` 데이터와 자기 workflow session만 접근 가능
- event trigger API(`/workflow-sessions/{thread_id}/events*`)와 debug API는 operator 전용
- customer는 자기 제안 확인, 메시지 기록, 승인/거절/수정 흐름만 사용한다

Push subscription 저장 API:
...
현재 구현은 subscription 등록/해제/조회까지만 담당한다. 실제 Web Push 발송은 VAPID
키와 발송 service를 추가하는 별도 단계다.
...
app/api/routes/
  HTTP API 표면이다. workflows.py가 agent workflow 실행 API이고,
  customers.py는 고객 선택과 화면용 데이터 조회 API다.
...
app/workflows/
  ...
  session_state.py는 고객 화면에 노출되는 작은 세션 상태만 정의한다.
...
코드와 문서의 기준은 `docs/redesign/`과 `docs/APIs/`다.
