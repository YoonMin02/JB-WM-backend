...
- **자산 → 건강/의료전략**: 가용 예산이 *치료 적극성·선택지*를 바꾼다. 자산 악화는 치료 미룸·스트레스로 이어져 *미래 건강 리스크*가 된다
- 그래서 같은 진단이라도 **자산 상태와 "지불 의향"에 따라 최적 전략이 달라진다**
...
→ 자산 상태 + 지불 의향(주관)은 '대응'의 개인화에만 반영
→ 재무·보장 대비 + 통계 기반 비용 참고정보 + 전문가 연결 제안 → 승인 → 액션
```
...
## 지불 의향 (willingness-to-pay) — 1급 개인화 변수

고객이 "의료/대응에 얼마까지 쓸 용의가 있는가"를 정합니다. 이것이:
- **개인화의 축**: 같은 상황도 지불 의향에 따라 다른 제안이 나온다.
- **의료조언 부담 해소 장치**: 회사는 "치료하세요"가 아니라 *"당신이 정한 예산·적극성 하에서 재무적으로 이렇게 대비됩니다"* 라고 말한다.
...
1. **재무·보장 대비** (본업)
2. **통계 기반 참고정보** (동일 질병 평균 비용·보장 사례 — 출처 명시, [STATS_SOURCES](STATS_SOURCES.md))
3. **전문가·기관 연결**
