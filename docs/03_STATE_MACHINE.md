...
| `HealthCareIntent` | "의료비를 어떻게 대비하지?" | 지불의향·자산 고려한 **의료비 감내 범위 설계** (의료 권고 아님) | 비용 범위별 재무 시나리오, 상담 질문 리스트, 통계 참고정보, 전문가 연결 제안 |
...
의료 관련 plan은 처치 선택을 권하지 않고, 의료진과 논의할 비용 범위와 그 범위별
재무 대응을 구조화합니다. 예: 기본 검사 중심, 추가 정밀검사 포함, 입원·시술 가능성까지
고려한 비용 범위. 각 범위는 고객의 현금흐름, 보험, 투자 유동화 필요성, 의료비 감내
범위와 함께 제시합니다.
