...
1. [`MVP_MOCK_IMPLEMENTATION_GUIDE.md`](MVP_MOCK_IMPLEMENTATION_GUIDE.md) — 현재 MVP에서 API 문서를 어떻게 써야 하는지
2. [`INTERNAL_ADAPTER_CONTRACTS.md`](INTERNAL_ADAPTER_CONTRACTS.md) — provider 원문을 내부 DTO로 바꾸는 기준
3. [`AGENT_TOOL_MAPPING.md`](AGENT_TOOL_MAPPING.md) — agent에게 노출되는 read tool 결과
4. 개별 원문 API 문서 — request/response body 항목, 타입, 날짜/금액 형식 참고
...
| `MVP_MOCK_IMPLEMENTATION_GUIDE.md` | 공통 | API 문서를 MVP mock 개발에 쓰는 방식 | 우선 참고 |
...
- [`MVP_MOCK_IMPLEMENTATION_GUIDE.md`](MVP_MOCK_IMPLEMENTATION_GUIDE.md) — 실제 연동 없이 mock/adapter를 만드는 기준
